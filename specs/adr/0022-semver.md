...
biocontext follows [Semantic Versioning 2.0.0](https://semver.org/) for its application releases and requires third-party plugins to do the same. The rule is simple: breaking changes increment the major version; everything else does not.
...
The top-level version of the biocontext distribution (the `version` field in `pyproject.toml`). This is what users install with `uv tool install biocontext` and what `biocontext --version` reports.
...
First-party plugins (shipped with biocontext) do not declare `PLUGIN_VERSION`. They are versioned implicitly by the application release version and are always mutually compatible.
